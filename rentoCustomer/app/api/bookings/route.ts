...
export async function POST(req: NextRequest) {
  ...
  // Mirror into the shop owner's portal FIRST so we have its id to link back —
  // best-effort: if this fails, the customer's own booking (source of truth for this
  // app) still gets created below, just without a linked owner-side record.
  const user = await getUserById(session.userId);
  const partnerBookingId = user
